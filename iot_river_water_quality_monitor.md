...
|-------|-----------|---------|
| **MCU** | ESP32 DevKit V1 | Dual-core Xtensa LX6 @ 240 MHz, 520 KB SRAM. Handles ADC reads, median filtering, LoRa TX. Deep-sleep between sampling cycles for power savings. |
...
| **Gateway → Cloud** | 4G USB Dongle (Jio/Airtel) | USB-serial to ESP32 via PPP or AT commands. Provides cellular backhaul where Wi-Fi is unavailable. SIM with 1.5 GB/day plan is sufficient for sensor telemetry. |
...
| **Charge Controller** | TP4056 Li-Ion charging module with DW01 protection IC. |
...
|-------|------------|---------|
| **Database** | Firebase Realtime Database | Stores time-series sensor data as `/{river}/{node_id}/{timestamp}`. Firestore used for metadata (node locations, calibration constants, alert configs). |
...
| **ML Inference** | TensorFlow Lite (on gateway ESP32) or Firebase Cloud Functions | TFLite model quantized to INT8 for edge inference. Fallback: Cloud Function triggers on new data write, runs full model, writes prediction back to DB. |
...
| Layer | Technology | Details |
|-------|------------|---------|
| **Firmware** | Arduino framework (PlatformIO) | Modular codebase: `sensors.h` (ADC reads + calibration), `lora.h` (packet TX/RX), `power.h` (sleep scheduling). OTA updates via ESP32 Wi-Fi (when gateway-proximate). |
